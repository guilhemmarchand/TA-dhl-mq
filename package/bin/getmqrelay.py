    ...
    def generate(self, **kwargs):
        ...
        if self:
            ...
            if str(mqpassthrough) == 'enabled':
                ...
                search = str(search) + " where status!=\"success\" AND ( (multiline=0 AND no_attempts>0) OR (multiline=1) ) | search " + str(kvstore_search_filters)
            output_mode = "csv"
            exec_mode = "oneshot"
            ...
            for row in readCSV:
                yield {'_time': time.time(), '_raw': str(row), '_key': str(row['_key']), 'message': str(row['message']), 'multiline': str(row['multiline']), 'appname': str(row['appname']), 'region': str(row['region']), 'ctime': str(row['ctime']), 'mtime': str(row['mtime']), 'manager': str(row['manager']), 'status': str(row['status']), 'queue': str(row['queue']), 'no_max_retry': str(row['no_max_retry']), 'no_attempts': str(row['no_attempts']), 'user': str(row['user']),}
